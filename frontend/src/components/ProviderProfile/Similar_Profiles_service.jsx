...
export default class Similar_Profiles_location extends React.Component {
  ...
  viewMore = () => {
    let i = 0;
    let finalArr = [];
    while (i < this.state.cityData.length) {
      if (
        this.props.rsn == this.state.cityData[i].servicesOffered[0].name &&
        this.state.cityData[i].fullName != this.state.providerData.fullName
      ) {
        finalArr.push(this.state.cityData[i]);
      }
      i = i + 1;
    }

    history.push(`/viewMore`, { criterion: finalArr });
  };
    ...
    return (
      <div>
        <div style={{ width: "90%", marginLeft: "12px" }}>
          More Profiles serving in {this.props.rsn}&nbsp;
          <ChevronRightIcon />
        </div>
        <br />
        ...
        this.state.cityData != undefined ? (
          this.state.cityData.map((itemz, idx) => {
            return (
              <div>
                {this.props.rsn == itemz.servicesOffered[0].name &&
                itemz.fullName != this.state.providerData.fullName ? (
                  <div>
                      ...
                      <div class="cardd" style={{ backgroundColor: "#FFF" }}>
                        <div class="header_element">
                          ...
                          <div class="right_header">
                            ...
                            >
                              ...
                              <span style={{ display: "none" }}>
                                {
                                  (share_link = `/provider/profile?id=${itemz.partnerId}`)
                                }
                              </span>
                              <span style={{ display: "none" }}>
                                {(share_email = `http://mailto:${itemz.email}`)}
                              </span>
                              <Link
                                style={{
                                  float: "right",
                        ...
                        <div class="cardd_padding">
                          ...
                          <Typography variant="caption" color="textSecondary">
                            ...
                            {itemz.partnerType.length > 1 ? (
                              <HtmlTooltip
                                ...
                                title={
                                  <React.Fragment>
                                    {itemz.partnerType != undefined &&
                                      itemz.partnerType.map((expertise) => {
                                        return (
                          ...
                          <Link
                            ...
                            class="link_hover"
                            style={{ color: "#4d4d4d" }}
                            to={`/provider/profile?id=${itemz.partnerId}`}
                          >
                            View Profile
                          </Link>
                          &nbsp;&nbsp;
                          ...
                          >
                            <span style={{ display: "none" }}>
                              {(modal_data_1 = itemz.email)}
                            </span>
                            <span style={{ display: "none" }}>
                              {(modal_data_2 = itemz.mobileNumber)}
                            </span>
                            <span style={{ display: "none" }}>
                              {(modal_data_3 = itemz.country)}
                            </span>
                            <span style={{ display: "none" }}>
                              {(modal_data_4 = itemz.approved)}
                            </span>
                            <span style={{ display: "none" }}>
                              {(modal_data_5 = itemz.City)}
                            </span>

                            <span style={{ display: "none" }}>
                              {(modal_data_6 = itemz.ALineOne)}
                            </span>
                            <span style={{ display: "none" }}>
                              {(modal_data_7 = itemz.ALineTwo)}
                            </span>
                            <span style={{ display: "none" }}>
                              {(modal_data_8 = itemz.PinCode)}
                            </span>
  ...
  render() {
    return <div>{this.logic_for_restricting_data()}</div>;
  }
}
